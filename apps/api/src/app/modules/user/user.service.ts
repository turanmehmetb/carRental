...
export class UserService extends BaseService<User> {
    ...
    findWithFilter(baseQuery: any): Promise<User[]> {
        ...
        const query: { where: any, order: any, skip: number, take: number } =
        {
          ...
          order: baseQuery.sortOrder ? {[baseQuery.sortField]: baseQuery.sortOrder} : { '_id': 1 }
        };

        query.where = this.getFilterQuery(baseQuery, query.where);

        // console.log("where" ,query.where, "-")
        return this.userModel.find().where().sort(query.order).limit(query.take).skip(query.skip).exec();
    }

    getFilterQuery(baseQuery: any, query: { db: number }) {
        baseQuery.userId ? query['userId'] = { $regex: baseQuery.userId, $options: 'i' } : null;
        baseQuery.name ? query['name'] = { $regex: baseQuery.name, $options: 'i' } : null;
        baseQuery.surname ? query['surname'] = { $regex: baseQuery.surname, $options: 'i' }  : null;
